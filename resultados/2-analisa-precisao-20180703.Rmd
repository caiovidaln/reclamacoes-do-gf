...
reclamacoes_aval_aleat = reclamacoes %>% select(insatisfacao)
...
reclamacoes_aval_aleat = reclamacoes_aval_aleat %>% 
    mutate(avaliacao_aleatoria = sample(1:5, 60, replace=T))

# Faça você mesmo. Crie a variável polaridade_normalizada
 reclamacoes_l = reclamacoes_l %>% 
     group_by(lexico) %>% 
     mutate(polaridade_normalizada = round((polaridade - min(polaridade)) / max(polaridade - min(polaridade)) * (1 - 5) + 5), digits = 0)
...
reclamacoes_aval_aleat = reclamacoes_aval_aleat %>% 
    mutate(erro_aleatorio = (insatisfacao - avaliacao_aleatoria)**2)


rmse_sent = reclamacoes_l %>% filter(lexico == 'sentimento_sent') %>% subset(select =  erro) %>% sum()

rmse_sent = sqrt(rmse_sent/60)

rmse_op = reclamacoes_l %>% filter(lexico == 'sentimento_op30') %>% subset(select =  erro) %>% sum()

rmse_op = sqrt(rmse_op/60)

rmse_al = reclamacoes_aval_aleat %>% select(erro_aleatorio) %>% sum()

rmse_al = sqrt(rmse_al/60)
